...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Info, ExternalLink, FileSpreadsheet, Share2 } from 'lucide-react';
...
export default function AppFull() {
  ...
  const getCSVContent = () => {
    ...
    // 格式化工具：使用標準雙引號包裹字串，解決 CSV 逗號分欄問題
    const fmt = (val, dec) => {
    ...
    if (navigator.share && navigator.canShare) {
        ...
        if (navigator.canShare(shareData)) {
            try {
                ...
                // 繼續執行下方的 downloadFile()
            }
        }
    }

    // 如果不支援分享，或分享失敗/取消，則執行直接下載
    downloadFile();
  };
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
 		 ...
 		 Fund 桌遊 v24.5 Full [{detectedEnv}]
		</div>
          </div>
      </div>
  );
        ...
        {gameStatus === 'ended' && (
            <div className="absolute inset-0 bg-slate-950/95 z-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                ...
                <div className="flex flex-col w-full max-w-xs gap-3">
                    ...
                    {/* LINE 提示 UI (v24.5: 因為支援原生分享，這個提示可以改為輔助用) */}
