...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Info, ExternalLink, FileSpreadsheet, Share2, Mail, MessageCircle, Monitor } from 'lucide-react';
...
export default function AppFull() {
  ...
  // V25.1 FIX: 移除 Excel 專用公式 (=)，改用標準 CSV 雙引號包裹
  // 這樣手機瀏覽器能正確顯示數值，且桌機 Excel 也能因為雙引號而正確分欄
  const fmt = (val, dec, useFormula = false) => {
    ...
    // 不再使用 "=""value""" 這種 Excel 專用寫法
    // 直接使用 "value" 這種標準 CSV 寫法
    return `"${s}"`;
  };
  ...
  const getCSVContent = () => {
    ...
    transactions.forEach(t => {
        ...
        const dateStr = currentDataPoint.date;
        const typeStr = t.type === 'BUY' ? '買入' : '賣出';
        // useFormula 現在僅作為標記，fmt 函式內部已經改為統一輸出標準格式
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
          <div className="w-full max-w-sm bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
              ...
              <h2 className="text-2xl font-bold text-white text-center mb-2">Fund 手遊</h2>
  ...
  return (
    <div style={containerStyle} className="bg-slate-950 text-slate-200 font-sans flex flex-col overflow-hidden transition-all duration-300">
        ...
        {/* V24.9 FIX: Z-Index increased to 60 to appear ABOVE the ended screen */}
        {confirmModal.show && (
                ...
                <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl shadow-2xl w-full max-w-xs text-center">
                    <div className="flex justify-center mb-4">
                        {confirmModal.type === 'exit' ? <LogOut size={40} className="text-slate-400"/> : 
                         ...
                         confirmModal.type === 'premium' ? <Monitor size={40} className="text-amber-400"/> : 
