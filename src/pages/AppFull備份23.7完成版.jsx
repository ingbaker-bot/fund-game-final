...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, FileText, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Info, ExternalLink, FileSpreadsheet, Share2 } from 'lucide-react';
...
export default function AppFull() {
  ...
  // v23.7 CSV 生成邏輯：支援 Native Share (LINE/Mail)
  const getCSVContent = () => {
  ...
  const generateAndShareCSV = async () => {
    ...
    if (navigator.share && navigator.canShare) {
        ...
        const shareData = {
            files: [file],
            title: '基金模擬戰報',
            text: `這是我的基金模擬戰報：${currentFundName}`
        };

        if (navigator.canShare(shareData)) {
            try {
                await navigator.share(shareData);
                return; // 如果分享成功，就停止後續的下載動作
            } catch (err) {
                if (err.name !== 'AbortError') {
                    console.error('Share failed:', err);
                } else {
                    return; // 使用者取消分享，不執行下載
                }
            }
        }
    }

    // 降級處理：如果是在電腦上或不支援分享，則執行傳統下載
    if (detectedEnv === 'Line' || detectedEnv === 'Facebook' || detectedEnv === 'Instagram') {
        alert("⚠️ 偵測到內建瀏覽器，若分享失敗請點擊「複製 CSV」或切換至 Chrome/Safari。");
    }
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
 		 ...
 		 Fund 桌遊 v23.7 Full [{detectedEnv}]
		</div>
          </div>
      </div>
  );
        ...
        {gameStatus === 'ended' && (
            <div className="absolute inset-0 bg-slate-950/95 z-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                ...
                <div className="flex flex-col w-full max-w-xs gap-3">
                    ...
                    {/* LINE 提示 UI (v23.7: 因為支援原生分享，這個提示可以改為輔助用) */}
                    ...
                    {/* v23.7 按鈕文字更新與功能升級 */}
                    <button onClick={generateAndShareCSV} className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 py-3.5 rounded-xl font-bold border border-slate-700 transition-colors text-sm">
