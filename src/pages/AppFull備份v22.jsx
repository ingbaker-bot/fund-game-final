...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, FileText, Maximize, Minimize, LogOut, Power, Lock, KeyRound, Database, User, UserCheck, Loader2, Waves, Settings2, Info } from 'lucide-react';
...
export default function AppFull() {
  ...
  // ★★★ V22.0 新增：河流圖進階設定 ★★★
  const [riverMode, setRiverMode] = useState('fixed'); // 'fixed'(固定%) 或 'dynamic'(標準差)
  const [riverWidthInput, setRiverWidthInput] = useState(10); // 固定模式的 % 數
  const [riverSDMultiplier, setRiverSDMultiplier] = useState(2); // 動態模式的標準差倍數 (通常是 2)
  ...
  const chartDataInfo = useMemo(() => {
    ...
    const slice = fullData.slice(start, end).map((d, idx) => {
        ...
        const ma20 = ind20.ma;
        const ma60 = ind60.ma;
        const stdDev60 = ind60.stdDev; // 季線的標準差

        // ★★★ v22.0 重點教學：河流圖 (布林通道概念) 計算公式 ★★★
        // 核心邏輯：利用「平均值 (MA)」加上「標準差 (SD)」來畫出價格波動的範圍
        let riverTop = null;
        ...
        if (ma60) {
            if (riverMode === 'fixed') {
                ...
                // 模式 B：動態標準差 (Fund 桌遊核心策略)
                // N = 60 (季線，代表過去一季的平均成本)
                // K = riverSDMultiplier (標準差倍數，通常設為 2 或 2.5)
                // 意義：約 95% 的價格波動會落在正負 2 倍標準差之內
                if (stdDev60) {
                    riverTop = ma60 + (stdDev60 * riverSDMultiplier);    // 上緣 = 均線 + (K * 標準差)
                    riverBottom = ma60 - (stdDev60 * riverSDMultiplier); // 下緣 = 均線 - (K * 標準差)
                }
            }
        }
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
 		 ...
 		 本系統採邀請制，如無帳號請洽ingbaker@gmail.com
  			<br />
			<br />
 		 Fund 桌遊 v22.0 版權所有 永名財務科技(股)
		</div>
          </div>
      </div>
  );
      ...
      <div className="min-h-screen bg-slate-950 text-white p-6 flex flex-col items-center justify-center font-sans">
        <div className="w-full max-w-sm bg-slate-900 rounded-xl p-6 shadow-2xl border border-slate-800 relative">
            ...
            <div className="text-center mb-6">
                <span className="bg-emerald-900 text-emerald-300 text-xs px-2 py-1 rounded border border-emerald-700">
                    v22.0 參數解鎖版
                </span>
            </div>
            ...
            <div className="bg-slate-900 p-3 rounded-lg border border-slate-700 mb-6">
                <div className="flex items-center justify-between mb-2 text-blue-400">
                    <div className="flex items-center gap-2">
                        <Waves size={16} />
                        <span className="text-xs font-bold uppercase tracking-wider">河流圖參數</span>
                    </div>
                    <span className="text-[10px] bg-blue-900/30 px-1.5 py-0.5 rounded text-blue-300">
                        N=60 (季線)
                    </span>
                </div>
                ...
                <div className="flex items-center bg-slate-800 border border-slate-600 rounded px-3 py-1">
                    {riverMode === 'fixed' ? (
                        <>
                            <span className="text-xs text-slate-400 mr-2">寬度</span>
                            <input type="number" value={riverWidthInput} onChange={(e) => setRiverWidthInput(Number(e.target.value))} className="flex-1 bg-transparent text-center text-white outline-none font-mono"/>
                            <span className="text-xs text-slate-400 ml-2">%</span>
                        </>
                    ) : (
                        <>
                            <span className="text-xs text-slate-400 mr-2" title="標準差倍數">K 值</span>
                            <input 
                                type="number" 
                                step="0.1" 
                                min="1" 
                                max="5" 
                                value={riverSDMultiplier} 
                                onChange={(e) => setRiverSDMultiplier(Number(e.target.value))} 
                                className="flex-1 bg-transparent text-center text-emerald-400 font-bold outline-none font-mono"
                            />
                            <span className="text-xs text-slate-400 ml-2">SD</span>
                        </>
                    )}
                </div>
                {/* 加入教學提示 */}
  ...
  return (
    <div style={containerStyle} className="bg-slate-950 text-slate-200 font-sans flex flex-col overflow-hidden transition-all duration-300">
        ...
        <div className="relative w-full bg-slate-900/30 border-b border-slate-800 shrink-0 z-0" style={{ height: '50%' }}>
            ...
            {showRiver && (<><Line type="monotone" dataKey="riverTop" stroke="#3b82f6" strokeWidth={2} dot={false} isAnimationActive={false} /><Line type="monotone" dataKey="riverBottom" stroke="#3b82f6" strokeWidth={2} dot={false} isAnimationActive={false} /></>)}
            {showMA20 && <Line type="monotone" dataKey="ma20" stroke="#fbbf24" strokeWidth={1} dot={false} isAnimationActive={false} opacity={0.8} />}{showMA60 && <Line type="monotone" dataKey="ma60" stroke="#a855f7" strokeWidth={1} dot={false} isAnimationActive={false} opacity={0.8} />}<Line type="monotone" dataKey="nav" stroke="#34d399" strokeWidth={2} dot={false} isAnimationActive={false} shadow="0 0 10px rgba(52, 211, 153, 0.3)" />{units > 0 && chartDataInfo.stopLossPrice && (<ReferenceLine y={chartDataInfo.stopLossPrice} stroke="#ef4444" strokeDasharray="3 3" strokeWidth={1.5} label={{ position: 'insideBottomLeft', value: `Stop ${chartDataInfo.stopLossPrice.toFixed(1)}`, fill: '#ef4444', fontSize: 10, dy: -5 }} />)}</ComposedChart></ResponsiveContainer>) : <div className="flex items-center justify-center h-full text-slate-500">載入中...</div>}
        </div>
