...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, FileText, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Info, ExternalLink, FileSpreadsheet } from 'lucide-react';
...
export default function AppFull() {
  ...
  // v23.6 CSV 產生邏輯 (Full Version 修復)
  const getCSVContent = () => {
    let csvContent = "交易日期,天數,類型,單價,單位數,交易金額,帳戶餘額,損益(賣出才有)\n";
    transactions.forEach(t => {
        // 安全獲取日期，避免 undefined
        const currentDataPoint = fullData[t.day];
        const dateStr = currentDataPoint?.date || `Day ${t.day}`;
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
 		 ...
 		 本系統採邀請制，如無帳號請洽管理員<br/>
 		 Fund 桌遊 v23.6 Full [{detectedEnv}]
		</div>
          </div>
      </div>
  );
