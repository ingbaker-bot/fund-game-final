...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Info, ExternalLink, FileSpreadsheet, Share2, Mail, MessageCircle, Monitor, Trophy, Globe, User, Sword } from 'lucide-react';
...
// ============================================
// V31.4: 主元件 AppRanked
// 更新: 修復手機版 Gmail 分享無法跳轉的問題 (Mobile Gmail Fix)
// ============================================
export default function AppRanked() {
  ...
  const getCSVContent = () => {
    ...
    transactions.forEach(t => {
        const currentDataPoint = fullData[t.day] || { date: 'N/A' };
        const row = `${currentDataPoint.date},${fmt(t.day, 2, true)},${t.type === 'BUY' ? '買入' : '賣出'},${fmt(t.price, 2, true)},${fmt(t.units, 2, true)},${fmt(t.amount, 0, true)},${fmt(t.balance, 0, true)},${t.type === 'SELL' ? fmt(t.pnl, 0, true) : '-'}`;
        csvContent += row + "\n";
    });
    return csvContent;
  };
  ...
  const executeUpload = async (nickname) => {
      setRankUploadStatus('uploading');
      setShowRankModal(true); // 確保 Modal 開啟以顯示 Loading
  ...
  const fetchAndShowLeaderboard = async () => {
      ...
      const data = await getLeaderboard(targetFundId);
      setLeaderboardData(data);
      setShowRankModal(true);
  };

  // --- V31.4: 改良版分享功能 (Mobile Gmail Fix) ---
  const handleShareAction = async (method) => {
    ...
    if (method === 'line') {
        ...
        link.download = `fund_game_${currentFundName}.csv`;
        ...
        // 2. 處理跳轉 (針對手機版優化)
        if (isMobile) {
            // 手機版: 使用 mailto 直接喚起 App (不彈窗，不延遲)
            // 這是手機上最穩定的做法，雖然不能自動帶入附件，但至少能開信箱
            window.location.href = `mailto:?subject=${subject}&body=${body}`;
        } else {
            // 桌機版: 彈窗提示並開啟 Gmail 網頁
            alert('檔案下載中... 請將下載的 Excel 檔案拖曳至 Gmail 附件區。');
            setTimeout(() => {
  ...
  if (!user) return ( 
      <div className="h-screen w-screen bg-slate-950 flex flex-col items-center justify-center font-sans p-6">
          <div className="w-full max-w-sm bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
              ...
              <h2 className="text-2xl font-bold text-white text-center mb-2">Fund 手遊 V31.4</h2>
              <p className="text-slate-400 text-center text-sm mb-6">社群爭霸版 - 登入</p>
              <form onSubmit={handleLogin} className="space-y-4">
                  <div><label className="text-xs text-slate-500 ml-1">Email</label><input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-emerald-500 outline-none"/></div>
                  <div><label className="text-xs text-slate-500 ml-1">密碼</label><input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-emerald-500 outline-none"/></div>
        ...
        <div className="w-full max-w-sm bg-slate-900 rounded-xl p-6 shadow-2xl border border-slate-800 relative">
            ...
            <div className="mb-4 mt-2">
                <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-amber-500 font-bold py-3 rounded-xl border border-amber-500/30 transition-all group"
                >
                    <Sword size={18} className="group-hover:rotate-12 transition-transform"/> 前往 S1 賽季競技場
                </button>
                <p className="text-[9px] text-slate-500 text-center mt-1">與其他玩家一較高下，爭奪榮耀！</p>
            </div>
            
            {tickerData.length > 0 && (
                <div className="mb-4 overflow-hidden bg-slate-950/50 border border-slate-800 rounded py-2">
                    <div className="whitespace-nowrap animate-marquee text-[10px] text-slate-400 px-2 flex gap-8">
                       {tickerData.map((tick, idx) => (
                           <span key={idx} className="flex items-center gap-1">
                               <span className="text-emerald-500 font-bold">★ {tick.displayName}</span> 
                               在 {tick.fundName.substring(0,6)}.. 獲利 
                               <span className="text-red-400 font-bold">+{tick.roi}%</span>
                           </span>
                       ))}
                    </div>
                </div>
            ...
            <div className="flex items-center justify-center gap-2 mb-4">
                ...
                {myNickname && <span className="text-amber-400 text-xs">({myNickname})</span>}
            </div>
                ...
                <div className="mb-3 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center gap-2 bg-slate-800 border border-slate-600 rounded-lg px-3 py-2">
                        ...
                        <select value={selectedFundId} onChange={(e) => setSelectedFundId(e.target.value)} className="w-full bg-transparent text-white outline-none text-sm">
                            {FUNDS_LIBRARY.map(fund => (<option key={fund.id} value={fund.id} className="bg-slate-900">{fund.name.replace('🔒 [進階] ', '')}</option>))}
                        </select>
                    </div>
                </div>
            )}
            ...
            <div className="text-center mt-4 mb-2">
                <span className="bg-amber-900/50 text-amber-400 text-xs px-2 py-1 rounded border border-amber-700/50 flex items-center justify-center gap-1 w-fit mx-auto">
                   <Trophy size={12}/> V31.4 (Mobile Gmail Fix)
                </span>
            </div>
        </div>
      </div>
  ...
  // ... (Game Playing Screen 保持不變，直接使用上方複製的內容) ...
  // (為了確保程式碼完整性，請使用上方第一段代碼框的完整內容)
  // ...
  return (
    // ... (這裡的內容已包含在上方完整代碼中)
                ...
                <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
                    ...
                    <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            <Trophy size={18} className="text-amber-400"/> 排行榜
                        </h3>
                        <button onClick={() => setShowRankModal(false)} className="text-slate-400 hover:text-white"><X size={20}/></button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                        {rankUploadStatus === 'asking_nick' && (
                            <div className="text-center py-6">
                                ...
                                <h4 className="text-xl font-bold text-white mb-2">初次見面！</h4>
                                <p className="text-slate-400 text-sm mb-6">請輸入您在江湖上的稱號 (日後將無法修改)</p>
                                <input 
                                    type="text" 
                                    value={inputNickname} 
                                    onChange={e => setInputNickname(e.target.value)}
                                    placeholder="例如：股海小童"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white text-center focus:border-emerald-500 outline-none mb-4"
                                    maxLength={12}
                                />
                                <button onClick={handleRegisterAndUpload} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold">
                                    確認並上傳成績
                                </button>
                            </div>
                        )}
                        ...
                        {rankUploadStatus === 'uploading' && (
                            <div className="flex flex-col items-center justify-center py-10">
                                ...
                                <p className="text-slate-400">正在將您的戰績刻入石碑...</p>
                            </div>
                        )}

                        {(rankUploadStatus === 'uploaded' || rankUploadStatus === 'idle') && leaderboardData.length > 0 && (
                            <div className="space-y-2">
                                <div className="flex justify-between text-[10px] text-slate-500 px-2 uppercase tracking-wider mb-1">
                                    <span>排名 / 玩家</span>
                                    <span>報酬率</span>
                                </div>
                                {leaderboardData.map((entry, idx) => {
                                    ...
                                    return (
                                        <div key={entry.id} className={`flex justify-between items-center p-3 rounded-lg border ${entry.uid === user.uid ? 'bg-emerald-900/20 border-emerald-500/50' : 'bg-slate-800/50 border-slate-800'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-6 h-6 flex items-center justify-center rounded-full font-bold text-xs ${idx===0 ? 'bg-amber-400 text-black' : (idx===1 ? 'bg-slate-400 text-slate-900' : (idx===2 ? 'bg-orange-700 text-orange-100' : 'bg-slate-700 text-slate-400'))}`}>
                                                    {idx + 1}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className={`text-sm font-bold ${entry.uid === user.uid ? 'text-emerald-400' : 'text-slate-200'}`}>
                                                        {entry.displayName}
                                                    </span>
                                                    <span className="text-[10px] text-slate-500">
                                                        {entry.fundName.substring(0, 10)}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className={`font-mono font-bold ${entry.roi >= 0 ? 'text-red-400' : 'text-green-400'}`}>
                                                    {entry.roi > 0 ? '+' : ''}{entry.roi}%
                                                </div>
                                                <div className="text-[10px] text-slate-500 flex items-center justify-end gap-1">
                                                    <span>${(entry.finalAssets/10000).toFixed(0)}萬</span>
            ...
            <div className="absolute inset-0 bg-slate-950/95 z-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
                ...
                <div className="flex flex-col w-full max-w-xs gap-3">
                    ...
                    <button onClick={handleInitiateUpload} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white py-4 rounded-xl font-bold shadow-lg shadow-amber-900/20 active:scale-[0.98] transition-all mb-2 animate-pulse">
                         <Globe size={18} /> {rankUploadStatus === 'uploaded' ? '查看目前排名' : '上傳戰績 / 爭奪排名'}
                    </button>

                    <button onClick={() => setShowShareMenu(true)} className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 py-3.5 rounded-xl font-bold border border-slate-700 transition-colors text-sm">
                    ...
                    <div className="mt-4 text-center text-[9px] text-slate-700">
                        V31.4 (Mobile Gmail Fix) | Environment: {detectedEnv}
                    </div>
                </div>
            </div>
        )}
        
        {/* Confirm Modal - 明確使用單一入口控制 */}
        {confirmModal.show && (
            <div className="absolute inset-0 bg-slate-950/80 z-[60] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl shadow-2xl w-full max-w-xs text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">
                        {confirmModal.type === 'exit' ? '離開遊戲' : (confirmModal.type === 'reset' ? '重置遊戲' : '結算遊戲')}
                    </h3>
                    <div className="flex gap-3 justify-center mt-6">
                         ...
                         <button 
                            onClick={() => {
                                if (confirmModal.type === 'exit') executeExit();
                                else if (confirmModal.type === 'reset') executeReset();
                                else executeEndGame(); // 預設為結算
                            }} 
                            className={`flex-1 py-3 rounded-xl font-bold text-white ${confirmModal.type === 'exit' ? 'bg-red-600 hover:bg-red-500' : (confirmModal.type === 'reset' ? 'bg-slate-600 hover:bg-slate-500' : 'bg-emerald-600 hover:bg-emerald-500')}`}
                        >
                            確定
                        </button>
                    </div>
            ...
            <div className="absolute inset-0 bg-slate-950/80 z-[60] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl shadow-2xl w-full max-w-sm text-center relative">
                    ...
                    <div className="flex flex-col gap-3 mt-4">
                        <button onClick={() => handleShareAction('line')} className="flex items-center justify-center gap-3 bg-[#06C755] hover:bg-[#05b54d] text-white py-3 rounded-xl font-bold transition-colors"><MessageCircle size={20} /> Line</button>
                        <button onClick={() => handleShareAction('gmail')} className="flex items-center justify-center gap-3 bg-white hover:bg-slate-100 text-red-600 py-3 rounded-xl font-bold transition-colors border border-slate-200"><Mail size={20} /> Gmail</button>
                        <button onClick={() => handleShareAction('download')} className="flex items-center justify-center gap-3 bg-slate-800 hover:bg-slate-700 text-slate-200 py-3 rounded-xl font-bold transition-colors border border-slate-700"><Download size={20} /> 下載 Excel</button>
                    </div>
                </div>
            </div>
        )}
