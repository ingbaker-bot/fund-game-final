...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Crown, Info, ExternalLink, FileSpreadsheet } from 'lucide-react';
...
export default function AppTrial() {
  ...
  // ★★★ v23.5 [修復] 補上遺漏的 CSV 產生邏輯 ★★★
  const getCSVContent = () => {
    let csvContent = "交易日期,天數,類型,單價,單位數,交易金額,帳戶餘額,損益(賣出才有)\n";
    transactions.forEach(t => {
        const dateStr = dataSourceType === 'real' ? fullData[t.day]?.date : `Day ${t.day}`;
        const typeStr = t.type === 'BUY' ? '買入' : '賣出';
        const pnlStr = t.type === 'SELL' ? t.pnl.toFixed(2) : '-';
        const row = `${dateStr},${t.day},${typeStr},${t.price.toFixed(2)},${t.units.toFixed(2)},${t.amount.toFixed(0)},${t.balance.toFixed(0)},${pnlStr}`;
        csvContent += row + "\n";
    });
    return csvContent;
  };
  ...
  const generateCSV = () => {
    ...
    // LINE/FB 瀏覽器防呆提示
    if (detectedEnv === 'Line' || detectedEnv === 'Facebook' || detectedEnv === 'Instagram') {
        alert("⚠️ 偵測到您正在使用 App 內建瀏覽器。\n若下載失敗，請點擊下方的「📋 複製 CSV 原始碼」按鈕，或切換至 Chrome/Safari 開啟。");
    }
    ...
    const fileName = `fund_game_${currentFundName}_${new Date().toISOString().slice(0,10)}.csv`;
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white p-6 flex flex-col items-center justify-center font-sans">
        <div className="w-full max-w-sm bg-slate-900 rounded-xl p-6 shadow-2xl border border-slate-800 relative">
            ...
            <div className="mt-4 text-center text-[9px] text-slate-700">Environment: {detectedEnv}</div>
        </div>
      </div>
    );
  }
    ...
    <div style={containerStyle} className="bg-slate-950 text-slate-200 font-sans flex flex-col overflow-hidden transition-all duration-300">
            ...
            <div className="absolute inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in duration-200">
                <div className="bg-slate-900 border border-slate-700 p-6 rounded-2xl shadow-2xl w-full max-w-xs text-center">
                    ...
                    <p className="text-slate-400 text-sm mb-6">
                        {confirmModal.type === 'exit' ? '確定要結束執行並關閉模擬器嗎？' : 
                         confirmModal.type === 'reset' ? '確定要放棄進度重新開始嗎？' : 
        ...
        <div className="flex flex-col w-full max-w-xs gap-3">
            <div className="bg-slate-800/50 p-3 rounded-xl border border-amber-500/30 mb-2">
                ...
                <button onClick={() => window.location.href = '/full'} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white py-3 rounded-lg font-bold shadow-lg text-sm transition-all"><Crown size={16} /> 免費註冊解鎖 20 檔基金</button>
            </div>
            ...
            {(detectedEnv === 'Line' || detectedEnv === 'Facebook' || detectedEnv === 'Instagram') && (
                <div className="bg-amber-900/30 border border-amber-600/50 rounded-xl p-3 text-center animate-pulse">
                    ...
                    <p className="text-xs text-amber-200/80">App 內建瀏覽器可能會擋檔案下載<br/>若失敗請用「複製 CSV」或切換至 Safari/Chrome</p>
                </div>
            )}

            <button onClick={generateCSV} className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 py-3.5 rounded-xl font-bold border border-slate-700 transition-colors text-sm"><Download size={16} /> 下載 Excel 明細</button>
            
            {/* v23.5 新增：強制複製 CSV 按鈕 */}
            ...
            <button onClick={copyToClipboard} className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 py-3.5 rounded-xl font-bold border border-slate-700 transition-colors text-sm">{showCopyToast ? <Check size={16} className="text-green-400"/> : <Copy size={16} />} {showCopyToast ? '已複製' : '複製純文字戰報'}</button><div className="h-6"></div><button onClick={executeReset} className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white py-4 rounded-xl font-bold shadow-lg shadow-emerald-900/20 active:scale-[0.98] transition-all"><RotateCcw size={18} /> 重新開始挑戰</button>
            
            {/* Debug Footer: 顯示當前偵測到的環境 */}
