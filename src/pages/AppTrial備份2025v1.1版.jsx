...
import { Play, Pause, TrendingUp, TrendingDown, Activity, RotateCcw, AlertCircle, X, Check, MousePointer2, Flag, Download, Copy, Maximize, LogOut, Power, Lock, Database, UserCheck, Loader2, Waves, Crown, Info, ExternalLink, FileSpreadsheet, Share2, Mail, MessageCircle, Monitor, Sword, Trophy, History } from 'lucide-react';
...
export default function AppTrial() {
  ...
  const [realStartDay, setRealStartDay] = useState(0); // 紀錄真實開始日，用於結算揭曉
  ...
  // 2025v1.1: 計算 Benchmark ROI (買入持有策略)
  ...
  const chartDataInfo = useMemo(() => {
    ...
    const slice = fullData.slice(start, end).map((d, idx) => {
        ...
        // 2025v1.1: 這裡將日期替換為偽裝後的日期，讓圖表顯示未來時間
  ...
  const startGame = async () => {
    ...
    // 2025v1.1: 設定真實起始日與 Benchmark 起始淨值
  ...
  const getCSVContent = () => {
    ...
    transactions.forEach(t => { 
        // 2025v1.1: CSV 匯出時也使用偽裝日期，保持一致性 (或可考慮匯出真實日期，這裡先選偽裝)
        ...
        <div className="w-full max-w-sm bg-white rounded-xl p-6 shadow-xl border border-slate-200 relative">
            ...
            <div className="mt-6 text-center">
                <span className="bg-slate-100 text-slate-500 text-xs px-3 py-1.5 rounded-full border border-slate-200 font-mono">
                    2025v1.1 體驗版 (Blind Test + Benchmark)
                </span>
            </div>
        </div>
      </div>
    ...
    <div style={containerStyle} className="bg-slate-50 text-slate-800 font-sans flex flex-col overflow-hidden transition-all duration-300">
        ...
        <div className="relative w-full bg-white border-b border-slate-200 shrink-0 z-0" style={{ height: '50%' }}>
            <div className="absolute top-3 left-4 z-0 pointer-events-none">
                <div className="flex items-baseline gap-3">
                    ...
                    {/* 2025v1.1: 這裡使用 getDisplayDate 顯示偽裝後的日期 */}
            ...
            {isReady && chartDataInfo.data.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartDataInfo.data} margin={{ top: 80, right: 5, left: 0, bottom: 5 }}>
                        ...
                        {/* 2025v1.1: X軸不顯示刻度，因為是偽裝的日期，避免太亂，Tooltip 會顯示偽裝日期 */}
        ...
        {gameStatus === 'ended' && (
            <div className="absolute inset-0 bg-white/95 z-50 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300 backdrop-blur-md">
                ...
                <p className="text-sm text-slate-500 mb-8 font-medium">(2025v1.1 體驗版結算)</p>
                ...
                {dataSourceType === 'real' && fullData[realStartDay] && fullData[currentDay] && (
                    <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl w-full max-w-xs mb-6 text-left">
                         ...
                         <div className="text-xs text-slate-600 space-y-1">
                             ...
                             <p>你經歷了：<span className="font-bold">{benchmarkRoi > 20 ? '📈 大牛市' : (benchmarkRoi < -20 ? '📉 熊市崩盤' : '⚖️ 盤整震盪')}</span></p>
                             <p>表現評語：<span className="font-bold text-slate-800">{roi > benchmarkRoi ? '🏆 你戰勝了大盤！' : '📚 下次試試長期持有？'}</span></p>
                         </div>
                    </div>
                )}
                ...
                <div className="flex flex-col w-full max-w-xs gap-3">
                    ...
                    <div className="mt-6 text-center text-[10px] text-slate-400">
                        Environment: {detectedEnv} | 2025v1.1 Blind Test Mode
                    </div>
                </div>
            </div>
        )}
